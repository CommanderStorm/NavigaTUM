...
export function useIrisAvailability(
  ...
  const poll = useIntervalFn(refresh, POLL_INTERVAL_MS, { immediate: false });
  ...
  // Refresh immediately on becoming active (and resume the interval); pause otherwise.
  watch(
    active,
    (isActive) => {
      if (isActive) {
  ...
  return { rooms, loading, failed: snapshot.failed };
}
